...
type WeatherBasedFoodButtonProps = {
  onClick: () => void
  weatherData: {
    main: { temp: number }
    weather: [{ main: string }]
...
const WeatherBasedFoodButton: React.FC<WeatherBasedFoodButtonProps> = ({ onClick, weatherData }) => {
  const backgroundImage = "https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&q=80&w=2070&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
  ...
  return (
    ...
    >
      <Image
        src={backgroundImage}
        alt="Weather-based food suggestions"
        layout="fill"
        objectFit="cover"
        ...
        {weatherData ? (
          <>
            <p className="text-white text-3xl font-bold text-center px-4 drop-shadow-lg mb-2">
              Discover Food for {weatherData.name}
            </p>
          ...
          <>
            <p className="text-white text-3xl font-bold text-center px-4 drop-shadow-lg mb-4">
              Enter a location to get food suggestions
            </p>
...
export default WeatherBasedFoodButton
